...
from .spider_test_suite import compute_test_beam_metric
...
class EvaluateTool(object):
        ...
        with open(dataset_filepath, encoding="utf-8") as f:
            ...
            for idx, sample in enumerate(dataset):
                if sample[
                    ...
                    sample['query_toks'] = ['SELECT', 'T1.company_type', 'FROM', 'Third_Party_Companies', 'AS', 'T1',
                                            'JOIN', 'Maintenance_Contracts', 'AS', 'T2', 'ON', 'T1.company_id', '=',
                                            'T2.maintenance_contract_company_id', 'ORDER', 'BY', 'T2.contract_end_date',
                                            'DESC', 'LIMIT', '1']
                    sample['query_toks_no_value'] = ['select', 't1', '.', 'company_type', 'from',
                                                     'third_party_companies', 'as', 't1', 'join',
                                                     'maintenance_contracts', 'as', 't2', 'on', 't1', '.', 'company_id',
                                                     '=', 't2', '.', 'maintenance_contract_company_id', 'order', 'by',
                                                     't2', '.', 'contract_end_date', 'desc', 'limit', 'value']
                    sample['question'] = 'What is the type of the company who concluded its contracts most recently?'
                    sample['question_toks'] = ['What', 'is', 'the', 'type', 'of', 'the', 'company', 'who', 'concluded',
                                               'its', 'contracts', 'most', 'recently', '?']
    ...
    def evaluate_beam(self, pred_candidate_sets, saved_result_path):
        exec_single_acc, exec_beam_acc = compute_test_beam_metric(pred_candidate_sets, self.golds, db_dir=None,
                                                                  saved_result_path=saved_result_path)
        return exec_single_acc, exec_beam_acc

    def spider_evaluate(self, preds, saved_result_path=None):
        db_dir = self.golds[0]['db_path']
        table = "./data/spider/tables.json"
        kmaps = spider_evaluation.build_foreign_key_map_from_json(table)
        etype = "all"
        evaluator = spider_evaluation.Evaluator(db_dir, kmaps, etype)
        for pred, ref in zip(preds, self.golds):
            # print(pred)
            # print(ref)
            # db_dir = f"{ref['db_path']}/{ref['db_id']}/{ref['db_id']}.sqlite"
            db_name = ref['db_id']
            gold = ref['query']
            evaluator.evaluate_one(db_name, gold, pred)
        evaluator.finalize()
        # print(evaluator.scores)
        spider_evaluation.print_scores(evaluator.scores, etype)
        return