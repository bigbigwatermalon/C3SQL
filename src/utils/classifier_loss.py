...
class ClassifierLoss():
    def __init__(self, alpha, gamma):
        ...
        self.ce_loss = nn.CrossEntropyLoss(weight=weight, reduction="mean", label_smoothing=0.15)
    ...
    def compute_batch_loss(self, batch_logits, batch_labels, batch_size):
        ...
        for logits, labels in zip(batch_logits, batch_labels):
            loss += self.focal_loss(logits, labels) # focal_loss
    ...
    def compute_reranker_loss(
            self,
            batch_sql_info_cls_logits,
            batch_sql_labels
    ):
        batch_size = len(batch_sql_labels)
        reranker_loss = self.compute_batch_loss(batch_sql_info_cls_logits, batch_sql_labels, batch_size)
