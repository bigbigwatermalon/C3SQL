...
def decode_natsqls(
    ...
    for batch_id in range(batch_size):
        ...
        print(f"##### db_path: {db_path}, db_id: {db_id}")
...
def decode_sql_candicates(
    ...
    for batch_id in range(batch_size):
        ...
        final_sql_candidate_sets.append(pred_sqls)

    return final_sql_candidate_sets


def decode_natsql_candidates(
        db_path,
        generator_outputs,
        batch_db_ids,
        batch_inputs,
    ...
    final_sql_candidate_sets = []
    natsql_candidate_sets = []
    natsql_fixed_candidate_sets = []
    ...
    for batch_id in range(batch_size):
        ...
        for seq_id in range(num_return_sequences):
            ...
            pred_natsql = pred_natsql.replace("='", "= '").replace("!=", " !=").replace(",", " ,")
            # if the predicted natsql has some fatal errors, try to correct it
            natsqls.append(pred_natsql)
            pred_natsql = fix_fatal_errors_in_natsql(pred_natsql, batch_tc_original[batch_id])
            natsqls_fixed.append(pred_natsql)
            pred_sql = natsql_to_sql(pred_natsql, db_id, db_file_path, table_dict[db_id]).strip()
        ...
        final_sql_candidate_sets.append(pred_sqls)
        natsql_candidate_sets.append(natsqls)
        natsql_fixed_candidate_sets.append(natsqls_fixed)

    # if os.path.exists("./results/natsql_candidate_sets.txt"):
    #     os.remove("./results/natsql_candidate_sets.txt")
    with open("./results/natsql_candidate_sets.txt", "a+") as f:
        for id, candicate_set in enumerate(natsql_candidate_sets):
            f.write(f"id: {id}\n")
            f.write("Correct\n")
            f.write("question\n")
            f.write("gold\n")
            for pred in candicate_set:
                f.write(pred + "\n")
            f.write("\n")
        f.write("\n")

    # if os.path.exists("./results/natsql_fixed_candidate_sets.txt"):
    #     os.remove("./results/natsql_fixed_candidate_sets.txt")
    with open("./results/natsql_fixed_candidate_sets.txt", "a+") as f:
        for id, candicate_set in enumerate(natsql_fixed_candidate_sets):
            f.write(f"id: {id}\n")
            f.write("Correct\n")
            f.write("question\n")
            f.write("gold\n")
